...
class CPTInterpreter : public CPerform
{
...
#pragma region *************** CONSTANT_PART_TO_EXTRACT ******************************
		...
		vector<CNode<string, float>*>* buffStackTree = new vector<CNode<string, float>*>();
			buffStackTree = pStackTree->at(IDX_OF_STACK_TREE);
...
#pragma region *************** CONSTANT_PART_TO_EXTRACT ******************************
		...
		while (aFirstDeq != '[' && end == false)
		{
			if (outChain->size() != 1)
			{
				...
				pStackTree->at(IDX_OF_STACK_TREE) = buffStackTree;
			}
			else
			{
				end = true;
				...
				if (outChain->size() == 1)
				{
					bufflag->at(IDX_OF_DONE_INTERPRET_SUBFLAG) = true;
...
//std::reverse(_treeChainL.begin(), _treeChainL.end());
//vector<CNode> _treeStack;

//string _buffLabel;
//if (_treeChainL.back() == ']')
//{
//	_treeChainL.pop_back(); // deq [
//	_treeChainL.pop_back(); // deq "

//	while (_treeChainL.back() != '"'){
//		_buffLabel.push_back(_treeChainL.back());
//		_treeChainL.pop_back();
//	}
//}